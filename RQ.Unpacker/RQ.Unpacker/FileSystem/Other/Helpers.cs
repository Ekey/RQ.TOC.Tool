...
namespace RQ.Unpacker
{
    public static class Helpers
    {
        ...
        public static string ReadUnicodeString(this Stream stream, Encoding encoding = null, bool trim = true, int defaultSize = 256)
        {
            encoding = encoding ?? Encoding.ASCII;
            ...
            int total = defaultSize - count * 2 - 1;
            stream.Position = stream.Position + total;
            var result = encoding.GetString(data.ToArray(), 0, count);
            return trim ? result.TrimEnd('\0') : result;
        }
